...
class OVSControl:
  ...
  def add_flow(self, bridge_name, flow):
    ...
    LOGGER.info(f'Adding flow {flow} to bridge: {bridge_name}')
  ...
  def bridge_exists(self, bridge_name):
    ...
    LOGGER.info(f'Checking if {bridge_name} exists')
  ...
  def verify_bridge(self, bridge_name, ports):
    LOGGER.info('Verifying bridge: ' + bridge_name)
    verified = True
    if self.bridge_exists(bridge_name):
      bridge_ports = self.get_bridge_ports(bridge_name)
      LOGGER.info('Checking bridge for ports: ' + str(ports))
      for port in ports:
